...
# from .transformer_module import MultiHeadAttentionJ, PositionwiseFeedForward
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, crs_mask=None):
        ...
        if crs_mask is not None:
            attn = attn.masked_fill(crs_mask == 0, float('-inf'))
...
class MultiheadAttentionJ(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, crs_mask=None):
        ...
        if crs_mask is not None:
            crs_mask = crs_mask.unsqueeze(1)

        attn_out, _ = self.attention(q, k, v, mask=mask, crs_mask=crs_mask)
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self,
        ...
        # self.attn = MultiheadAttentionJ(num_heads, embed_dims, embed_dims // num_heads, embed_dims // num_heads, dropout=attn_drop, qkv_bias=False)

        self.proj_drop = nn.Dropout(proj_drop)
        self.dropout_layer = nn.Dropout(dropout_layer['drop_prob']) if dropout_layer else nn.Identity()
        ...
        for _ in range(num_fcs - 1):
            layers.append(
                nn.Sequential(
                    ...
                    nn.ReLU(inplace=True), nn.Dropout(ffn_drop)))
