...
class FlashCrossAttention(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if flash_attn_varlen_kvpacked_func is None and not gpc.config.common.npu:
            import warnings
            warnings.warn(
                f"flash_attn not available ({import_error_msg}); falling back to scaled_dot_product_attention.",
                stacklevel=2,
            )
    ...
    def gpu_flash(
        ...
        if flash_attn_varlen_kvpacked_func is not None:
            ...
            # fallback: standard scaled dot-product attention (no flash_attn required)
            q_s = rearrange(q, '(b s) h d -> b h s d', b=bsz)
            k_s = rearrange(kv[:, 0], '(b s) h d -> b h s d', b=bsz)
            v_s = rearrange(kv[:, 1], '(b s) h d -> b h s d', b=bsz)
