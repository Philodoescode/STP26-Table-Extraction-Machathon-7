...
class SparseSelfAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_naiive or tgt_len != src_len:
            if self.use_fa_v2:
    ...
    def fwd_naiive_v2(
        ...
        d = q.shape[-1]
        attn_weights = torch.einsum('bthd,bshd->bhts', q / math.sqrt(d), k)
        
        if use_mask:
            if seq_lengths is None:
                fullmask = get_smooth_local_mask(seqlen_q, seqlen_k, self.local_size, device=q.device)
        ...
        output = torch.einsum('bhts,bshd->tbhd', attention_probs, v)
