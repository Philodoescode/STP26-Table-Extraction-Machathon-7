...
from TDATR.models.modules.mappings import scatter_to_sequence_parallel_region
...
class EmbeddingEx_cfgi(nn.Module):
    def __init__(
        self,
        embed_dim,
        vocab_size,
        ...
        super(EmbeddingEx_cfgi, self).__init__()
        ...
        # Embeddings dropout
        self.embedding_dropout = torch.nn.Dropout(embedding_dropout_prob)
        self.cfgi_posi = nn.Sequential(
                    nn.Linear(4, self.embed_dim//2),
                    nn.ReLU(),
                    nn.Linear(self.embed_dim//2, self.embed_dim )
                )
    def forward_(self, tokens, row_position, save_mem=False):
        # Embeddings.
        

        words_embeddings = self.word_embeddings(tokens)
        
        row_max,_ = torch.max(row_position[:,:,0:2], dim=-1)
        col_max,_ = torch.max(row_position[:,:,2:3], dim=-1)
        row_max = row_max
        col_max = col_max
        max_span = torch.stack([ row_max, row_max, col_max, col_max ], dim=-1)
        posi = row_position/(1e-5+max_span.to(tokens))
        embeddings = words_embeddings + self.cfgi_posi(posi)
