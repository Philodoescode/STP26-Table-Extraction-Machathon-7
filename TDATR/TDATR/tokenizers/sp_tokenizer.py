...
class SPTokenizer(object):
    @staticmethod
    def _resolve_vocab_prefix(vocab_prefix: str) -> str:
        prefix_path = Path(vocab_prefix)
        ...
        candidates = [
            Path.cwd() / prefix_path,
            project_root / prefix_path,
            module_root / prefix_path,
        ]
        for candidate in candidates:
            candidate = candidate.resolve()
            if (candidate.with_suffix(".vocab")).exists() and (candidate.with_suffix(".model")).exists():
                return str(candidate)

        return str((Path.cwd() / prefix_path).resolve())

    def __init__(self, cfg):
        resolved_vocab_prefix = self._resolve_vocab_prefix(cfg.vocab_file)
        model_file = resolved_vocab_prefix + ".model"
        vocab_file = resolved_vocab_prefix + ".vocab"
        logger.info("\n\nSPTokenizer_vocab_file:{}".format(resolved_vocab_prefix))
